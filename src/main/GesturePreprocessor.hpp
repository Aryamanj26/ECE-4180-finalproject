...
struct GestureEpisode
{
    ...
    // when each sensor first or last saw the object in this episode
    uint32_t firstSeenMs[3]= { 0, 0, 0 };
    uint32_t lastSeenMs[3]= { 0, 0, 0 };

    // peak approach velocity in mm per second toward sensors per sensor
...
public:
    ...
    GestureEvent update(uint16_t d0, uint16_t d1, uint16_t d2, uint32_t nowMs)
    {
        ...
        // find the closest valid sensor reading in this frame
        ...
        // check which sensors are valid based on being close to the nearest object
        // this prevents background objects from interfering with gesture detection
        ...
        switch (state)
        {
            case State::Idle:
                ...
                if (anyValid)
                {
                    // need consecutive valid frames before we start tracking
            ...
            case State::Tracking:
                if (anyValid)
                {
                    ...
                    // check if episode has gone on too long
                ...
                {
                    // hand left sensor field so finish the episode
     ...
     */
    const GestureEpisode &lastEpisode() const
    {
        return ep;
    }
...
private:
    ...
    // distance band for your 8 to 14 cm gesture plane
    static constexpr uint16_t D_MIN_MM= 30;
    static constexpr uint16_t D_MAX_MM= 140;

    // loosen start and stop so episodes actually happen
    static constexpr uint8_t  ENTER_COUNT= 1;
    static constexpr uint8_t  EXIT_COUNT= 2;

    // looser episode duration
    static constexpr uint32_t MIN_EPISODE_MS= 20;
    static constexpr uint32_t MAX_EPISODE_MS= 2000;

    // looser radial movement requirement we can tighten later
    static constexpr uint16_t MIN_SWING_MM= 5;
    ...
    // how many consecutive invalid frames before we clear filt for that sensor
    ...
    void filterDistances(const uint16_t raw[3])
    {
        ...
        // first compute m for each sensor with preference for current valid raw
        for (int i= 0; i < 3; i++)
        {
            ...
            {
                // invalid reading use median of history
                m[i]= median3(rawHist[i][0], rawHist[i][1], rawHist[i][2]);
            }
        }

        // find the closest valid reading aka closest object to the sensors
        // this way the sensors are not influenced by the background or noise when making a gesture
        uint16_t zMinFrame= 0xFFFF;
        for (int i= 0; i < 3; i++)
        {
            if (m[i]== 0 || m[i]== 0xFFFF)
        ...
        // for each sensor only keep values in the global band determined above
        // and within its area determined by near layer threshold
    ...
    void appendSample(const bool valid[3], uint32_t nowMs)
    {
        ...
        for (int i= 0; i < 3; i++)
        {
            ...
            if (valid[i])
            {
                // mark first and last seen times for this sensor in this episode
                if (ep.firstSeenMs[i]== 0)
                {
                    ep.firstSeenMs[i]= nowMs;
                }
                ep.lastSeenMs[i]= nowMs;

                // compute per sample approach velocity if we have a previous sample
                // velocity is positive when hand moves closer to sensors
                if (lastFiltForVel[i] != 0 && d != 0 && lastTimeForVel[i] != 0)
                {
                    uint32_t dt= nowMs - lastTimeForVel[i];
                    if (dt > 0)
                    {
                        // distance change greater than zero means moving closer
                        int16_t dv= (int16_t)lastFiltForVel[i] - (int16_t)d;
                        if (dv > 0)
                        {
                            // convert to mm per second
                            int16_t v= (int16_t)((dv * 1000) / (int32_t)dt);
                            if (v > ep.maxApproachVel[i])
                            {
                                ep.maxApproachVel[i]= v;
                            }
                        }
                    }
                }
        ...
        // track which sensor sees the closest object and count changes
        if (winner >= 0)
        {
    ...
    bool finalizeEpisode(uint32_t nowMs)
    {
        ...
        // find the maximum swing from any sensor
        // swing is how much the distance changed during the gesture
        uint16_t maxSwing= 0;
        for (int i= 0; i < 3; i++)
        {
            if (ep.dMin[i] != 0xFFFF)
        ...
        // gesture needs either decent swing or decent velocity to be valid
